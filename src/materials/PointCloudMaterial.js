...
export class PointCloudMaterial extends THREE.RawShaderMaterial {
	constructor(parameters = {}) {
		...
		this._shadow = "F1_chequered_flag.png"
	...
	set shadow(value) {
		if (this._shadow !== value) {
			this._shadow = value;
			this.shadowTexture = Potree.PointCloudMaterial.generateShadowTexture(this._shadow);
			this.uniforms.shadowTextureUniform.value = this.shadowTexture;
		}
	}
	...
	get zoneCategory() {
		return this.uniforms.zoneCategory.value;
	}

	set zoneCategory(mode) {
		this.uniforms.zoneCategory.value = mode;
	}
	...
	get xrange() {
		return this.uniforms.xrange.value;
	}

	set xrange(value) {
		let changed = this.uniforms.xrange.value[0] !== value[0]
			|| this.uniforms.xrange.value[1] !== value[1];

		if (changed) {
			this.uniforms.xrange.value = value;
	...
	get yrange() {
		return this.uniforms.yrange.value;
	}

	set yrange(value) {
		let changed = this.uniforms.yrange.value[0] !== value[0]
			|| this.uniforms.yrange.value[1] !== value[1];

		if (changed) {
			this.uniforms.yrange.value = value;
	...
	get zrange() {
		return this.uniforms.zrange.value;
	}

	set zrange(value) {
		let changed = this.uniforms.zrange.value[0] !== value[0]
			|| this.uniforms.zrange.value[1] !== value[1];

		if (changed) {
			this.uniforms.zrange.value = value;
	...
	static generateShadowTexture(shadow) {
		var url = new URL(Potree.resourcePath + "/textures/shadow/" + shadow).href;
		let texture = new THREE.TextureLoader().load(url);
