...
#define ZONECATEGORY_NONE 0
#define ZONECATEGORY_GO 1
#define ZONECATEGORY_NOGO 2

uniform int clipTask;
uniform int zoneCategory;// remove
...
#endif
//
#if defined(num_noneboxes)&&num_noneboxes>0
uniform mat4 noneBoxes[num_noneboxes];
#endif
#if defined(num_greenboxes)&&num_greenboxes>0
uniform mat4 greenBoxes[num_greenboxes];
#endif
#if defined(num_redboxes)&&num_redboxes>0
uniform mat4 redBoxes[num_redboxes];
#endif
...
float getLOD(){
	...
	for(float i=0.;i<=30.;i++){
		float nodeSizeAtLevel=uOctreeSize/pow(2.,i+uLevel+0.);
		
		vec3 index3d=(position-offset)/nodeSizeAtLevel;
		index3d=floor(index3d+.5);
		int index=int(round(4.*index3d.x+2.*index3d.y+index3d.z));
		
		vec4 value=texture2D(visibleNodes,vec2(float(iOffset)/2048.,0.));
		int mask=int(round(value.r*255.));
		
		if(isBitSet(mask,index)){
			// there are more visible child nodes at this position
			int advanceG=int(round(value.g*255.))*256;
			int advanceB=int(round(value.b*255.));
			...
			float lodOffset=(255.*value.a)/10.-10.;
			
			return depth+lodOffset;
		}
		
		offset=offset+(vec3(1.,1.,1.)*nodeSizeAtLevel*.5)*index3d;
	}
...
float getSpacing(){
	...
	for(float i=0.;i<=30.;i++){
		...
		if(i>0.){
			spacing=spacing/(255.*spacingFactor);
		}
...
float getLOD(){
	...
	for(float i=0.;i<=1000.;i++){
		...
		vec4 value=texture2D(visibleNodes,vec2(iOffset/2048.,0.));
		
		int children=int(value.r*255.);
		float next=value.g*255.;
		int split=int(value.b*255.);
		
		if(next==0.){
			return depth;
		}
		
		vec3 splitv=vec3(0.,0.,0.);
		if(split==1){
			splitv.x=1.;
		}else if(split==2){
			splitv.y=1.;
...
float getContrastFactor(float contrast){
	return(1.0158730158730156*(contrast+1.))/(1.0158730158730156-contrast);
}
...
vec3 getRGB(){
	...
	rgb=rgb+uRGB_gbc.y;
	rgb=(rgb-.5)*getContrastFactor(uRGB_gbc.z)+.5;
	rgb=clamp(rgb,0.,1.);
...
float getIntensity(){
	...
	w=(w-.5)*getContrastFactor(uIntensity_gbc.z)+.5;
	w=clamp(w,0.,1.);
...
vec3 getGpsTime(){
	...
	vec3 c=texture2D(gradient,vec2(w,1.-w)).rgb;
...
vec3 getElevation(){
	...
	vec3 cElevation=texture2D(gradient,vec2(w,1.-w)).rgb;
	
	return cElevation;
}

vec4 getClassification(){
	vec2 uv=vec2(classification/255.,.5);
...
vec3 getReturns(){
	...
	// 0b 00_000_111
	float rn=mod(returnNumber,8.);
	// 0b 00_111_000
	float nr=mod(returnNumber/8.,8.);
...
vec3 getNumberOfReturns(){
	...
	vec3 color=texture2D(gradient,vec2(w,1.-w)).rgb;
	
	return color;
}

vec3 getSourceID(){
	float w=mod(pointSourceID,10.)/10.;
	return texture2D(gradient,vec2(w,1.-w)).rgb;
}
...
vec3 getCompositeColor(){
	...
	c+=wIntensity*getIntensity()*vec3(1.,1.,1.);
	w+=wIntensity;
	...
	if(w==0.){
		//c = color;
		gl_Position=vec4(100.,100.,100.,0.);
	}
...
vec3 getNormal(){
	...
	vec3 n_view=normalize(vec3(modelViewMatrix*vec4(normal,0.)));
	return n_view;
}
...
vec3 getShadow(){
	...
	vec2 uv=(modelMatrix*vec4(position,1.)).xy;
	
	// Bring it to [0, 1]
	uv.x=(uv.x-xrange.x)/(xrange.y-xrange.x);
	uv.y=(uv.y-yrange.x)/(yrange.y-yrange.x);
	
	return texture2D(shadowTextureUniform,uv).rgb+vec3(.1,.1,.1);
}
#endif
...
vec3 getColor(){
	...
	color=(1.-uTransition)*getRGB()+uTransition*cHeight;
	#elif defined color_type_depth
	float linearDepth=gl_Position.w;
	float expDepth=(gl_Position.z/gl_Position.w)*.5+.5;
	color=vec3(linearDepth,expDepth,0.);
	...
	float w=getIntensity();
	color=texture2D(gradient,vec2(w,1.-w)).rgb;
	...
	color=(modelMatrix*vec4(normal,0.)).xyz;
...
float getPointSize(){
	...
	float slope=tan(fov/2.);
	float projFactor=-.5*uScreenHeight/(slope*vViewPosition.z);
	...
	if(uUseOrthographicCamera){
		float worldSpaceSize=1.*size*r/getPointSizeAttenuation();
...
bool pointInClipPolygon(vec3 point,int polyIdx){
	...
	vec4 pointNDC=wvp*vec4(point,1.);
...
void doClipping(){
	...
	#if defined(num_noneboxes)&&num_noneboxes>0
	for(int i=0;i<num_noneboxes;i++){
		vec4 clipPosition=noneBoxes[i]*modelMatrix*vec4(position,1.);
		
		bool inside=-.5<=clipPosition.x&&clipPosition.x<=.5;
		inside=inside&&-.5<=clipPosition.y&&clipPosition.y<=.5;
		inside=inside&&-.5<=clipPosition.z&&clipPosition.z<=.5;
		
		insideNone=insideNone+(inside?1:0);
	}
	#endif
	
	#if defined(num_greenboxes)&&num_greenboxes>0
	for(int i=0;i<num_greenboxes;i++){
		vec4 clipPosition=greenBoxes[i]*modelMatrix*vec4(position,1.);
		
		bool inside=-.5<=clipPosition.x&&clipPosition.x<=.5;
		inside=inside&&-.5<=clipPosition.y&&clipPosition.y<=.5;
		inside=inside&&-.5<=clipPosition.z&&clipPosition.z<=.5;
		
		insideGreen=insideGreen+(inside?1:0);
	}
	#endif
	
	#if defined(num_redboxes)&&num_redboxes>0
	for(int i=0;i<num_redboxes;i++){
		vec4 clipPosition=redBoxes[i]*modelMatrix*vec4(position,1.);
		
		bool inside=-.5<=clipPosition.x&&clipPosition.x<=.5;
		inside=inside&&-.5<=clipPosition.y&&clipPosition.y<=.5;
		inside=inside&&-.5<=clipPosition.z&&clipPosition.z<=.5;
		
		insideRed=insideRed+(inside?1:0);
		
	}
	#endif
	
	// bool insideAny = insideCount > 0;
	bool yellow=insideNone>0;
	bool green=insideGreen>0;
	bool red=insideRed>0;
...
void main(){
	...
	float adjustedDepth=originalDepth+2.*vRadius;
	...
	for(int i=0;i<num_clipspheres;i++){
		...
		if(distance<1.){
			float w=distance;
			vec3 cGradient=texture2D(gradient,vec2(w,1.-w)).rgb;
	...
	const float sm_near=.1;
	const float sm_far=10000.;
	
	for(int i=0;i<num_shadowmaps;i++){
		vec3 viewPos=(uShadowWorldView[i]*vec4(position,1.)).xyz;
		...
		vec2 sampleStep=vec2(1./(2.*1024.),1./(2.*1024.))*1.5;
		vec2 sampleLocations[9];
		sampleLocations[0]=vec2(0.,0.);
		sampleLocations[1]=sampleStep;
		sampleLocations[2]=-sampleStep;
		sampleLocations[3]=vec2(sampleStep.x,-sampleStep.y);
		sampleLocations[4]=vec2(-sampleStep.x,sampleStep.y);
		
		sampleLocations[5]=vec2(0.,sampleStep.y);
		sampleLocations[6]=vec2(0.,-sampleStep.y);
		sampleLocations[7]=vec2(sampleStep.x,0.);
		sampleLocations[8]=vec2(-sampleStep.x,0.);
		
		float visibleSamples=0.;
		float numSamples=0.;
		
		float bias=vRadius*2.;
		...
		if(u<0.||u>1.||v<0.||v>1.||nc.x<-1.||nc.x>1.||nc.y<-1.||nc.y>1.||nc.z<-1.||nc.z>1.){
			//vColor = vec3(0.0, 0.0, 0.2);
		}else{
			//vColor = vec3(1.0, 1.0, 1.0) * visibility + vec3(1.0, 1.0, 1.0) * vec3(0.5, 0.0, 0.0) * (1.0 - visibility);
			vColor=vColor*visibility+vColor*uShadowColor*(1.-visibility);
		}
