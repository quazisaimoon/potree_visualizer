...
import { Tag, TagBox } from "../../utils/Tag.js";
...
export class PropertiesPanel {
  ...
  setPointCloud(pointcloud) {
    ...
    let panel = $(`
			<div class="scene_content selectable">
				...
				<div style="display: none" class="divider">
					<span>Attribute</span>
				</div>
				...
				<div style="display: none" id="materials.rgb_container">
					<div style="display: none" class="divider">
						<span>RGB</span>
					</div>
  ...
  setMeasurement(object) {
    ...
    let getType = (measurement) => {
      if (measurement instanceof Measure) {
        ...
        return TYPE.PROFILE;
      } else if (measurement instanceof Region) {
        return TYPE.REGION;
      } else if (measurement instanceof Volume) {
        return TYPE.VOLUME;
      } else if (measurement instanceof Tag) {
        return TYPE.TAG;
      }
    };
  ...
  setPath(path) {
    let panel = new DistancePanel(this.viewer, path, this);
    this.container.append(panel.elContent);
  }

  setArea(area) {
    let panel = new AreaPanel(this.viewer, area, this);
    this.container.append(panel.elContent);
  }
