...
import { TagBox } from "../utils/Tag";
import { Features } from "../Features.js";
import { Message } from "../utils/Message.js";
import { Sidebar } from "./sidebar.js";

import { AnnotationTool } from "../utils/AnnotationTool.js";
import { PathTool } from "../utils/PathTool.js";
import { AreaTool } from "../utils/AreaTool.js";
import { MeasuringTool } from "../utils/MeasuringTool.js";
import { ProfileTool } from "../utils/ProfileTool.js";
import { RegionTool } from "../utils/RegionTool";
import { VolumeTool } from "../utils/VolumeTool.js";
import { TagTool } from "../utils/TagTool";
...
export class Viewer extends EventDispatcher {
    ...
    try {
      {
        ...
        if ($(domElement).find("#pcd_loading").length === 0) {
          const pcdLoading = $(
            `<h3 id="pcd_loading" class="pcd_loading_hide">Loading a pointcloud. It might take a while.</h3>`
          );
          $(domElement).append(pcdLoading);
        }
        ...
        const elToolbar = $(`
				<div id="potree_toolbar">
					<span>
						<div class="potree_toolbar_label">Measurement Tools</div>
						<div class="potree_toolbar_tool_container">
							<img 
								name="action_measure_angle" 
								src="${Potree.resourcePath}/icons/angle.png" 
							...
							<img 
								name="action_measure_point" 
								src="${Potree.resourcePath}/icons/point.svg" 
								class="annotation-action-icon" 
								style="width: 2em; 
								height: auto;"
							/>
							...
							<img 
								name="action_measure_distance" 
								src="${Potree.resourcePath}/icons/distance.svg" 
								class="annotation-action-icon" 
								style="width: 2em; 
								height: auto;"
							/>
							...
							<img 
								name="action_measure_height" 
								src="${Potree.resourcePath}/icons/height.svg" 
								class="annotation-action-icon" 
								style="width: 2em; 
								height: auto;"
							/>
							...
							<img 
								name="action_measure_azimuth" 
								src="${Potree.resourcePath}/icons/azimuth.svg" 
								class="annotation-action-icon" 
								style="width: 2em; 
								height: auto;"
							/>
							...
							<img 
								...
								src="${Potree.resourcePath}/icons/area.svg" 
								class="annotation-action-icon" 
								style="width: 2em; 
								height: auto;"
							/>
        ...
        // Add to the HTML above if you want it
        // Look at the toolbar.html for examples
        {
          // GRADIENT
          ...
          const elGradientSchemes = elToolbar.find(
            "span[name=gradient_schemes]"
          );
        ...
        {
          // MEASUREMENT TOOLS
          elToolbar.find("img[name=action_measure_angle]").click(() => {
          ...
          elToolbar.find("img[name=action_measure_area]").click(() => {
            const measurement = viewer.measuringTool.startInsertion({
              showDistances: true,
              showArea: true,
              closed: true,
      ...
      this.zoneCategory = ZoneCategory.NONE;
...

      {
        ...
        // this.setZoneCategory(ZoneCategory.GO);
      ...
      this.regionTool = new RegionTool(this);
  ...
  setZoneCategory(value) {
    // TODO: enable this once it's possible to change color for individual region instances
  ...
  loadGUI(callback) {
    ...
    // let sidebarContainerRight = $('#potree_sidebar_container_right');

    // Right sidebar
    $(document.body).append(`
			<div id='potree_sidebar_right_wrapper'>
				<div id="potree_sidebar_right_container">
					<div class="divider">Mission Specification</div>
					<div id="mission_spec_details">

						<label for="mission_spec_title">Title:</label>
						<input 
							value=""
							type="text" 
							id="mission_spec_title" 
							class="form_input" 
							name="mission_spec_title"
						>

						<label for="mission_spec_description">Description:</label>
						<textarea
							rows="2"
							id="mission_spec_description" 
							class="form_input form_textarea" 
							name="mission_spec_description"></textarea>

						<label for="mission_spec_start_date">Start date:</label>
						<input
							class="form_input" 
							type="date" 
							id="mission_spec_start_date" 
							name="mission_spec_start_date"
						>

            <label for="mission_spec_start_time">Start time:</label>
            <input
              class="form_input" 
              type="time" 
              id="mission_spec_start_time"
              name="mission_spec_start_time"
            >
					...
					<div style="display: none" class="divider">Metadata</div>
					<div style="display: none" id="scene_object_metadata"></div>
				</div>
			</div>
    ...
    // event listeners
    document
      .getElementById("mission_spec_title")
      .addEventListener(
        "change",
        (e) => (this.missionSpecs.missionTitle = e.target.value)
      );

    document
      .getElementById("mission_spec_description")
      .addEventListener(
        "change",
        (e) => (this.missionSpecs.missionDescription = e.target.value)
      );

    document
      .getElementById("mission_spec_start_date")
      .addEventListener(
        "change",
        (e) => (this.missionSpecs.missionStartDate = e.target.value)
      );

    document
      .getElementById("mission_spec_start_time")
      .addEventListener(
        "change",
        (e) => (this.missionSpecs.missionStartTime = e.target.value)
      );
...

    {
      ...
      boxes.push(
        ...this.scene.volumes.filter(
          (v) => (v.clip && v instanceof BoxVolume) || v instanceof TagBox
        )
      );
      ...
      for (let region of this.scene.regions) {
        boxes.push(...region.boxes);
      }
      ...
      for (let pointcloud of visiblePointClouds) {
        ...
        pointcloud.material.zoneCategory = this.zoneCategory;
        pointcloud.material.clipMethod = this.clipMethod;
        // console.log(pointcloud.material.zoneCategory)
      }
    }
