...
export class Utils {
	...
	static createMetadataLabel(text, id = null) {
		const label = new TextSprite();
		label.setBorderColor({r: 0, g: 0, b: 0, a: 1.0});
	...
	static generateName(prefix) {
		return `${prefix}_${THREE.Math.generateUUID().slice(0,8)}`;
	}
	...
	static getPathIcon(path) {

		if(path instanceof Path) {
			if(path._closed && path.showArea) {
				// return `${Potree.resourcePath}/icons/area_tool.svg`;
			} else if (path._closed) {
				return `${Potree.resourcePath}/icons/poi.svg`;
			} else {
				return `${Potree.resourcePath}/icons/path.svg`;
			}
		}
	...
	static getRegionIcon(region) {
		return `${Potree.resourcePath}/icons/region_tool.svg`;
	}

	static getTagIcon(tag) {
		return `${Potree.resourcePath}/icons/tag.svg`;
	}
